...
actual class BigInteger: Number, Comparable<BigInteger> {
    ...
    internal constructor(value: String, radix: Int = 10) {
        var cursor = 0
        val numDigits: Int
    ...
    actual fun shortValueExact(): Short {
     ...
     * @see .shiftLeft
     */
    actual infix fun shiftRight(n: Int): BigInteger {
        return if (this.signum == 0) {
            ZERO
        } else if (n > 0) {
            shiftRightImpl(n)
     ...
     * @return `this ^ other`
     */
    actual infix fun xor(other: BigInteger): BigInteger {
        val result = IntArray(maxOf(intLength(), other.intLength()))
        for (i in result.indices) {
     ...
     */
    @JsName("not")
    actual operator fun not(): BigInteger {
        val result = IntArray(intLength())
        for (i in result.indices) {
     ...
     *
     * @param  augend value to be added to this BigInteger.
     * @return `this + augend`
     */
    @JsName("add")
    actual fun add(augend: BigInteger): BigInteger {
        ...
        if (augend.signum == this.signum) {
            return BigInteger(add(this.mag, augend.mag), this.signum)
        }
        val cmp = compareMagnitude(augend)
        if (cmp == 0) {
            return ZERO
        }
        var resultMag = if (cmp > 0) subtract(this.mag, augend.mag)
                else subtract(augend.mag, this.mag)
     ...
     *
     * @param  subtrahend value to be subtracted from this BigInteger.
     * @return `this - val`
     */
    @JsName("subtract")
    actual fun subtract(subtrahend: BigInteger): BigInteger {
        ...
        if (subtrahend.signum != this.signum) {
            return BigInteger(add(this.mag, subtrahend.mag), this.signum)
        }
        val cmp = compareMagnitude(subtrahend)
        if (cmp == 0) {
            return ZERO
        }
        var resultMag: IntArray = if (cmp > 0) subtract(this.mag, subtrahend.mag)
                else subtract(subtrahend.mag, this.mag)
        resultMag = trustedStripLeadingZeroInts(resultMag)
        return BigInteger(resultMag, if (cmp == this.signum) 1 else -1)
    }
     ...
     *
     * @param  multiplicand value to be multiplied by this BigInteger.
     * @return `this * value`
     */
    @JsName("multiply")
    actual fun multiply(multiplicand: BigInteger): BigInteger {
        if (multiplicand.signum == 0 || this.signum == 0) {
            return ZERO
        }
        val xlen = mag.size
        val ylen = multiplicand.mag.size

        if (xlen < AlgorithmUtils.KARATSUBA_THRESHOLD || ylen < AlgorithmUtils.KARATSUBA_THRESHOLD) {
            val resultSign = if (this.signum == multiplicand.signum) 1 else -1
            if (multiplicand.mag.size == 1) {
                return multiplyByInt(mag, multiplicand.mag[0], resultSign)
            }
            if (mag.size == 1) {
                return multiplyByInt(multiplicand.mag, mag[0], resultSign)
            }
            var result = multiplyToLen(mag, xlen,
                    multiplicand.mag, ylen, null)
            ...
            return if (xlen < AlgorithmUtils.TOOM_COOK_THRESHOLD && ylen < AlgorithmUtils.TOOM_COOK_THRESHOLD) {
                AlgorithmUtils.multiplyKaratsuba(this, multiplicand)
            } else {
                AlgorithmUtils.multiplyToomCook3(this, multiplicand)
            }
        }
    }
     ...
     */
    @JsName("mod")
    actual fun mod(divisor: BigInteger): BigInteger {
        if (divisor.signum <= 0) {
            throw ArithmeticException("BigInteger: modulus not positive")
        }
        val result = this % divisor
        return if (result.signum >= 0) result else (result + divisor)
    }
    ...
    fun modPow(exponent: BigInteger, m: BigInteger): BigInteger {
        ...
        val base = if (this.signum < 0 || this >= m) this % m else this
        ...
        if (m.testBit(0)) { // odd modulus
            ...
            val base2 = if (this.signum < 0 || this >= m1) this % m1 else this
